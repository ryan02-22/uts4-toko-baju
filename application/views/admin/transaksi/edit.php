...
<div class="edit-transaksi-wrapper">
    <div class="form-title">
        <h2><i class="fas fa-edit"></i> Edit Transaction #<?= $transaksi->id; ?></h2>
    </div>

    <form method="POST" action="<?= base_url('admin/update_transaksi/' . $transaksi->id); ?>">
        ...
        <div class="form-section">
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <input type="text" value="<?= $transaksi->customer_name ?? 'N/A'; ?>" readonly>
                </div>
                <div class="form-group">
                    <label>Transaction Date</label>
                    <input type="text" value="<?= date('d M Y, H:i', strtotime($transaksi->transaction_date)); ?>" readonly>
                </div>
            </div>
            ...
            <div class="form-row full">
                <div class="form-group">
                    ...
                    <textarea name="shipping_address" required><?= $transaksi->shipping_address; ?></textarea>
                </div>
            </div>
        </div>
            ...
            <div class="product-list">
                ...
                <div id="items-container">
                    ...
                    foreach ($details as $detail): ?>
                        <div class="product-item" id="item-<?= $itemIndex; ?>">
                            <select name="product_id[]" class="product-select" onchange="updateProductPrice(<?= $itemIndex; ?>)" required>
                                <option value="">-- Select Product --</option>
                                <?php foreach ($produk as $p): ?>
                                    <option value="<?= $p->id; ?>" data-price="<?= $p->price; ?>" <?= $p->id == $detail->product_id ? 'selected' : ''; ?>>
                                        <?= $p->name; ?>
                                    </option>
                                <?php endforeach; ?>
                            </select>
                            <input type="number" name="quantity[]" value="<?= $detail->quantity; ?>" min="1" class="qty-input" onchange="calculateSubtotal(<?= $itemIndex; ?>)" required>
                            <input type="number" name="price[]" value="<?= $detail->price; ?>" class="price-input" readonly>
                            <input type="number" class="subtotal-input" value="<?= $detail->subtotal; ?>" readonly>
                            <button type="button" class="btn-remove-item" onclick="removeItem(<?= $itemIndex; ?>)">Remove</button>
                        </div>
                    <?php $itemIndex++;
                    endforeach; ?>
                </div>
            ...
            <div class="form-row full">
                <div class="form-group">
                    ...
                    <div style="padding: 12px; background: white; border: 1px solid var(--border-color); border-radius: 4px; font-size: 18px; font-weight: 700; color: var(--secondary-color);">
                        Rp <span id="total-amount"><?= number_format($transaksi->total_amount, 0, ',', '.'); ?></span>
                    </div>
                </div>
            </div>
